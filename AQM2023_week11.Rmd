...
```{r}
mean_satis <- mean(dta$satislfe_survey_mean)
satis_high <- mean_satis + sd(dta$demeaned_satis)

scenario1 <- c(1, mean_satis, rep(0, 14))
scenario2 <- c(1, satis_high, rep(0, 14))

ev1 <- sim_function(lm_obj = fe_1,
                    nsim = 10000,
                    scenario = scenario1)
ev2 <- sim_function(lm_obj = fe_1,
                    nsim = 10000,
                    scenario = scenario2)
fd <- ev2 - ev1
...
```{r}
mean_satis_f <- mean(dta$satislfe_survey_mean)
satis_high_f <- mean_satis_f + sd(dta$satislfe_survey_mean)

scenario1_f <- c(1, mean_satis_f, rep(0, 14))
scenario2_f <- c(1, satis_high_f, rep(0, 14))

ev1_f <- sim_function(lm_obj = fe_1,
                      nsim = 10000,
                      scenario = scenario1_f)
ev2_f <- sim_function(lm_obj = fe_1,
                      nsim = 10000,
                      scenario = scenario2_f)

fd_f <- ev2_f - ev1_f

mean_fd_f <- mean(fd_f)
ci_fd_f <- quantile(fd_f, c(0.025, 0.975))
```
...
plot(x = 0,
     ...
     bty = "n",
     las = 1,
     main = "First Difference (one SD within)",
     ylab = "First Difference",
     xlab = "",
     xaxt = "n")
segments(x0 = 0, x1 = 0,
         y0 = ci_fd[1], y1 = ci_fd[2],
         lwd = 2)
abline(h = 0,
       lty = "dashed")
...
plot(x = 0,
     ...
     bty = "n",
     las = 1,
     main = "First Difference (one SD total)",
     ylab = "First Difference",
     xlab = "",
     xaxt = "n")
segments(x0 = 0, x1 = 0,
         y0 = ci_fd_f[1], y1 = ci_fd_f[2],
         lwd = 2)
abline(h = 0,
       lty = "dashed")
